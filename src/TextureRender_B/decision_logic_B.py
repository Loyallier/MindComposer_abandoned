...
def _consolidate_chords(
    chord_sequence: List[str], quantization_step: float = 0.25
...
def generate_accompaniment_part(
    ...
    for i in range(num_chords):
        ...
        if chord_type == "NoChord":
            logger.debug(
                f"和弦 {chord_label} 为 NoChord/休止，跳过该片段，不生成伴奏。"
            )
            current_offset += segment_length
            continue
        ...
        logger.debug(
            f"和弦 {chord_label}, 时长 {segment_length:.2f} QL, 织体 {pattern_name}"
        )
        ...
        while time_elapsed_in_segment < segment_length:
            ...
            if (
                time_elapsed_in_segment > segment_length
                and time_elapsed_in_segment - template_duration == last_note_end_time
            ):
                break
